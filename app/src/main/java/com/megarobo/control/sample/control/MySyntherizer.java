package com.megarobo.control.sample.control;
...
import com.megarobo.control.sample.MainHandlerConstant;
...
public class MySyntherizer implements MainHandlerConstant {
    ...
    protected boolean init(InitConfig config) {
        ...
        mSpeechSynthesizer.setParam(SpeechSynthesizer.PARAM_URL,"https://tsn.baidu.com/text2audio");
        if (isMix) {

            // 授权检测接口(只是通过AuthInfo进行检验授权是否成功。选择纯在线可以不必调用auth方法。
            sendToUiThread("auth begin");

            AuthInfo authInfo = mSpeechSynthesizer.auth(config.getTtsMode());
            sendToUiThread("auth end");
