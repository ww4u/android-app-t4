...
public class UDPThread1 extends Thread {
	...
	public UDPThread1(String target_ip) {
...
//				packet = new DatagramPacket(NBREQ, NBREQ.length, address, NBUDPP);
				packet = new DatagramPacket(new byte[0], 0,address, NBUDPP);
				socket = new DatagramSocket();
				socket.setSoTimeout(200);
				socket.send(packet);
				socket.close();
				ARPManager.count++;
				if(ARPManager.count == 255){
					EventBus.getDefault().postSticky(new IPSearchEvent("ip search completed!"+ARPManager.count));
				}
